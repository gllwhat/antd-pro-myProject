...
function Comments ({ data }) {
    const columns = [
        {
            ...
            dataIndex: 'avatar',
            width: 48,
            className: styles.avatarcolum,
            render: text => <span style={{ backgroundImage: 'url(${text})' }}></span>            
        },
        {
            title: 'content',
            dataIndex: 'content',
    ...
    return (
        <div>
            <Table pagination={false} showHeader={false} columns={columns} rowKey={(record, key) => key} dataSource={data.filter((item, key) => key < 3)}></Table>
        </div>
    )
}
