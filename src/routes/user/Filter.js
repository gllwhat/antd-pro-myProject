...
const Filter = ({
    ...
    // form表单自带的字段过滤API
    form: {
        // 基础函数 想要用getFieldsValue等 必须使用 
        // 用于和表单进行双向绑定
        getFieldDecorator,
        // 获取form的value值 同e.target.value
        getFieldsValue,
    ...
    const handleFields = (fields) => {
        ...
        // createTime[0].format('YYYY-MM-DD') 起始时间
        // createTime[1].format('YYYY-MM-DD') 结束时间
    ...
    // 处理提交
    const handleSubmit = () => {
        // getFieldsValue 获取value的值
        let fields = getFieldsValue()
        fields = handleFields(fields)
        // onFilterChange 当 Form.Item 子节点的值发生改变时触发，可以把对应的值转存到 Redux store
